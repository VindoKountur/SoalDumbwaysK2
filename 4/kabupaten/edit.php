...
<body>
  ...
  <?php 
    include '../koneksi.php';
    $id = $_GET['id'];
    $sqlGet = "SELECT * FROM kabupaten_tb WHERE id = '$id'";
  ...
  ?>
    ...
    <form action="" method="post" class='form__tambah' enctype="multipart/form-data">
      ...
      <label>Provinsi</label><select name="provinsi_id">
        <?php 
          while ($dataProvinsi = $queryProvinsi->fetch_assoc()){
            if ($dataProvinsi['id'] == $dataGet['provinsi_id']) {
              echo "
                <option value='$dataProvinsi[id]' selected>$dataProvinsi[nama]</option>
              ";  
            } else {
              echo "
...
<?php 
  if (isset($_POST['nama'])) {
    ...
    $sql = "UPDATE kabupaten_tb SET nama = '$nama', diresmikan = '$diresmikan', photo = '$photo', provinsi_id = '$provinsi_id' WHERE id = '$id'";
